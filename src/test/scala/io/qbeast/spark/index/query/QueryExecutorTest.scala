...
class QueryExecutorTest extends QbeastIntegrationTestSpec with QueryTestSpec {
  ...
  it should "filter cube blocks by query weight range" in
    withSparkAndTmpDir((spark, tmpDir) => {
      ...
      val rootBlock = Block(tmpDir, CubeId(2, root.string), Weight(0.0), Weight(0.1), 1)
      val c1Block1 = Block(tmpDir, CubeId(2, c1.string), Weight(0.1), Weight(0.2), 1)
      val c1Block2 = Block(tmpDir, CubeId(2, c1.string), Weight(0.7), Weight(0.8), 1)
